...
output_csv_file = "data/experiments/benchmark/cryosr_benchmark_sample.csv"
output_dir = "data/experiments/benchmark/cryosr_sample"
batch_size = 40

def run_cryosr(input_map, output_map, batch_size):
    try:
        start = datetime.now()
        ...
        print("Failed to generate map using cryosr")
        print(e)
        return 0
...
if __name__ == "__main__":
    ...
    for id in emd_list:
        ...
        output_map = os.path.join(emdb_output_dir, "cryosr_generated.mrc")
        os.makedirs(emdb_output_dir, exist_ok=True)
        runtime = 0

        if os.path.exists(input_map):
            print("Processing EMD", emdb_id)
            runtime = run_cryosr(input_map, output_map, batch_size)
